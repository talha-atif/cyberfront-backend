...
def build_and_run_container(container_name, dockerfiles_folder="."):
    ...
    if not os.path.isfile(dockerfile_full_path):
        output.append(f"No Dockerfile found for {container_name} in {dockerfiles_folder}. Creating a basic Dockerfile...")
    ...
    # Running the new container with resource limits
    # Here, I set the CPU shares to 0.5 and memory limit to 256m as an example
    cpu_shares = "2.0"
    memory_limit = "256m"
    run_cmd = f"docker run --name {container_name} -d --cpus={cpu_shares} --memory={memory_limit} {image_name}"
